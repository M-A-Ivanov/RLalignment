...
class oLeem(object):
    ...
    def testConnect(self):
        if self.Leem2000Connected:
            ...
            try:
                ...
                print(
                    'please check: that LEEM is running')
                print(
                    '              the IP address is correct')
                print(
                    '              the PORT is the same activated in LEEM2000')
                return False
            else:
                return False
    ...
    def setIP(self, IP):
        if self.Leem2000Connected:
            print(
                'Already connected ... close connection first')
            return
        else:
            if type(IP) != str:
                print(
        ...
        if not self.Leem2000Connected:
            ...
            if data != '0':
                ...
                for i in range(nChanges):
                    Changes.append({'moduleName': self.Modules[int(arr[i * 2])], 'moduleNr': int(arr[i * 2]),
